...
// Função para salvar tweets no Local Storage
const saveTweetsToLocalStorage = (tweets: Tweet[]) => {
    localStorage.setItem('tweets', JSON.stringify(tweets));
};

// Função para carregar tweets do Local Storage
...
export function MenuConteudo({ children }: MenuProps) {
    ...
    const [tweets, setTweets] = useState<Tweet[]>(loadTweetsFromLocalStorage()); // Carregar tweets do Local Storage
    ...
    const handleTweet = () => {
        ...
        const newTweet: Tweet = {
            ...
            date: new Date(),
            comments: [], // Inicializa com uma lista vazia de comentários
            likes: 0,
        };
                    ...
                    {tweets.map((tweet) => (
                        <TweetItem key={tweet.id}>
                            <div>
                                ...
                                <div>
                                    <strong>{tweet.username}</strong> • {formatDistanceToNow(new Date(tweet.date), { addSuffix: true, locale: ptBR })}
                                </div>
                            </div>
                            <p>{tweet.content}</p>
